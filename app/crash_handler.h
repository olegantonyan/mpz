...
// Registers the file that each crash trace is appended to, separated by a banner. Call once after
// the application name is set. The parent directory is created lazily on the first crash, not here.
...
// Registers a pre-rendered system-info block written into each crash item. Call once at startup after the application version is set. Safe to omit.
void set_system_info(std::string info);

// Lifecycle phase written into each crash item. String literals only: kept by pointer and read from a signal handler.
