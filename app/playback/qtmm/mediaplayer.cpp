...
namespace Playback {
  namespace Qtmm {
  MediaPlayer::MediaPlayer(quint32 stream_buffer_size, QByteArray outdevid, QObject *parent) : Playback::MediaPlayer(parent), stream(stream_buffer_size), output_device_id(outdevid), next_after_stop(true), next_after_stop_cue(true) {
  ...
  QList<Playback::MediaPlayer::AudioDevice> MediaPlayer::audioOutputs() {
    ...
    for (const auto &device : std::as_const(devices)) {
      AudioDevice ad;
      ad.id = device.id();
      ad.description = device.description();
      ad.is_default = device.isDefault();
      out.append(ad);
    }
#endif
    return out;
  ...
  void MediaPlayer::seek_to_offset_begin() {
    QTimer timer;
    QEventLoop loop;
