...
namespace DiskCache {
  // A hashed file cache under QStandardPaths::CacheLocation, shared by the online lyrics and album cover caches so the two can't drift apart.
  //
  // Unlike the scratch dirs in CoverArt::Embedded/Mpd this is never wiped on exit — it is
  // the point of the thing — so it is capped instead, and cleared only when the user asks.
  class Store {
  public:
    ...
    // Negative results are tagged with the provider set that produced them, so enabling another provider retries instead of inheriting the old miss.
    ...
    // Stable key for a two-part identity (artist + album, artist + title). Empty when either part is empty, which disables caching for that item.
